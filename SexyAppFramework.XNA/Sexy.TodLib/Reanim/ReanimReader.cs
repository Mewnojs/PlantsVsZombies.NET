    ...
    internal class ReanimReader : ContentTypeReader<ReanimatorDefinition>
    {
        ...
        private void ReadReanimTransform(CustomContentReader input, ReanimScaleType doScale, bool isGround, out ReanimatorTransform transform)
        {
            ...
            else
            {
                ...
                {
                    transform.mTransX = ((num == ReanimatorXnaHelpers.DEFAULT_FIELD_PLACEHOLDER) ? num : FrameworkConstants.InvertAndScale(num));
                    transform.mTransY = ((num2 == ReanimatorXnaHelpers.DEFAULT_FIELD_PLACEHOLDER) ? num2 : FrameworkConstants.InvertAndScale(num2));
                }
                else if (doScale == ReanimScaleType.ScaleFromPC) 
                {
                    transform.mTransX = ((num == ReanimatorXnaHelpers.DEFAULT_FIELD_PLACEHOLDER) ? num : FrameworkConstants.S * num);
                    transform.mTransY = ((num2 == ReanimatorXnaHelpers.DEFAULT_FIELD_PLACEHOLDER) ? num2 : FrameworkConstants.S * num2);
                }
                else
                {
                    transform.mTransX = num;
