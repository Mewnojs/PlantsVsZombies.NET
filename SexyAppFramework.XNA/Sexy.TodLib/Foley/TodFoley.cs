    ...
    public/*internal*/ class TodFoley
    {
        ...
        public void PlayFoleyPitch(FoleyType theFoleyType, float aPitch)
        {
            ...
            if (TodCommon.TestBit(foleyParams.mFoleyFlags, (int)FoleyFlags.OneAtATime))
            {
                ...
                {
                    foleyInstance.mRefCount++;
                    foleyInstance.mStartTime = SexyGlobal.gSexyAppBase.mUpdateCount;
                    return;
                }
            }
            ...
            SoundInstance soundInstance = SexyGlobal.gSexyAppBase.mSoundManager.GetSoundInstance((uint)theSfxID);
            ...
            foleyInstance2.mStartTime = SexyGlobal.gSexyAppBase.mUpdateCount;
            foleyTypeData.mLastVariationPlayed = num3;
            if (aPitch != 0f)
            {
        ...
        {
            if (SexyGlobal.gSexyAppBase.mSfxVolume < 9.999999974752427E-07)
            {
                theFoleyInstance.mInstance.SetVolume(0.0);
                return;
            }
            double volume = SexyGlobal.gSexyAppBase.mMusicVolume * SexyGlobal.gSexyAppBase.mSfxVolume;
        ...
        public static bool SoundSystemHasFoleyPlayedTooRecently(TodFoley theSoundSystem, FoleyType theFoleyType)
        {
            ...
            if (foleyTypeData != null)
            {
                for (int i = 0; i < TodLibConstants.MAX_FOLEY_INSTANCES; i++)
                {
                    ...
                    if (foleyInstance.mRefCount != 0)
                    {
                        int num = SexyGlobal.gSexyAppBase.mUpdateCount - foleyInstance.mStartTime;
                        if (num < 2)
                        {
                            return true;
                        }
