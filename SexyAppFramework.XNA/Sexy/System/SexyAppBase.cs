...
namespace Sexy
{
    public/*internal*/ class SexyAppBase : SexyAppBaseInterface, ButtonListener, DialogListener, IDisposable
    {
        ...
        public static string mLanguage
        {
            get
            {
                return FrameworkConstants.Language.ToString();
            }
        }
        ...
        public/*internal*/ virtual void NewGame()
        {
        }
        ...
        internal static void GetWidthHeightForOrientation(UI_ORIENTATION theOrientation, ref int theWidth, ref int theHeight)
        {
            ...
            {
                if (FrameworkConstants.Loaded)
                {
                    theWidth = FrameworkConstants.BackBufferSize.X;
                    theHeight = FrameworkConstants.BackBufferSize.Y;
                    return;
                }
                theWidth = SexyGlobal.g.GraphicsDevice.PresentationParameters.BackBufferWidth;
                theHeight = SexyGlobal.g.GraphicsDevice.PresentationParameters.BackBufferHeight;
                return;
            }
            else
            {
                if (FrameworkConstants.Loaded)
                {
                    theWidth = FrameworkConstants.BackBufferSize.Y;
                    theHeight = FrameworkConstants.BackBufferSize.X;
                    return;
                }
                theWidth = SexyGlobal.g.GraphicsDevice.PresentationParameters.BackBufferWidth;
                theHeight = SexyGlobal.g.GraphicsDevice.PresentationParameters.BackBufferHeight;
        ...
        {
            SexyGlobal.g.BeginFrame();
            mWidgetManager.DrawScreen();
            SexyGlobal.g.EndFrame();
        }
        ...
        public virtual void InterfaceOrientationChanged(UI_ORIENTATION toOrientation)
        {
            ...
            XnaGame.NeedToSetUpOrientationMatrix(toOrientation);
        ...
        public/*internal*/ bool Is3DAccelerated()
        {
            return true;
        }
        ...
        public void ShowUpdateRequiredMessage()
        {
            ...
            IMain.GamerServicesComp.Enabled = false;
        }
...
#pragma warning restore CS0618
        ...
        public void LockOrientation(bool theFlag)
        {
            ...
            {
                GraphicsState.mGraphicsDeviceManager.SupportedOrientations = FrameworkConstants.SupportedOrientations;
            }
            if (SexyAppBase.XnaGame.Window.CurrentOrientation == DisplayOrientation.Portrait)
            {
                GraphicsState.mGraphicsDeviceManager.PreferredBackBufferWidth = FrameworkConstants.BackBufferSize.X;
                GraphicsState.mGraphicsDeviceManager.PreferredBackBufferHeight = FrameworkConstants.BackBufferSize.Y;
            }
            else
            {
                GraphicsState.mGraphicsDeviceManager.PreferredBackBufferWidth = FrameworkConstants.BackBufferSize.Y;
                GraphicsState.mGraphicsDeviceManager.PreferredBackBufferHeight = FrameworkConstants.BackBufferSize.X;
            }
        ...
        {
            string text;
            if (!SexyAppBase.cachedIntToString.TryGetValue(i, out text))
            {
                text = i.ToString();
                SexyAppBase.cachedIntToString.Add(i, text);
            }
            return text;
        }
        ...
        public static bool IsInTrialMode
        {
            get
            {
                return IMain.IsInTrialMode;
            }
        }

        private const double VIBRATION_DURATION = 500.0;

        public static IMain XnaGame;
