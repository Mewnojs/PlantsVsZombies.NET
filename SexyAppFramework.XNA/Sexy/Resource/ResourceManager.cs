...
namespace Sexy
{
    public/*internal*/ class ResourceManager : IDisposable
    {
        ...
        private Texture2D LoadTextureFromStream(string filename, bool premultiply, ImageRes.TextureFormat format, SurfaceFormat lowMemorySurfaceFormat)
        {
            ...
            {
                texture2D = Texture2D.FromStream(SexyGlobal.g.GraphicsDevice, stream);
            }
            ...
            else
            {
                ...
                {
                    SexyGlobal.gSexyAppBase.mTexturesToBePremultiplied.Push(texture2D);
                }
                else 
                { 
        ...
        public/*internal*/ Texture2D PremultiplyTexture(/*SurfaceFormat lowMemorySurfaceFormat, */Texture2D texture2D, bool doRefTexture)
        {
            bool flag = false;
            GraphicsDevice graphicsDevice = SexyGlobal.g.GraphicsDevice;
            lock (ResourceManager.DrawLocker)
            {
                if (texture2D.Width * texture2D.Height < 4194304)
                {
                    ...
                    try
                    {
                        ...
                        {
                            if (IMain.DO_LOW_MEMORY_OPTIONS)
                            {
            ...
            try
            {
                ...
                if (theRes.mLanguageSpecific)
                {
                    text = string.Concat(new string[]
                    {
                        ...
                        FrameworkConstants.ImageSubPath,
                        FrameworkConstants.LanguageSubDir,
                        "/",
                ...
                {
                    text = Path.GetDirectoryName(theRes.mPath) + FrameworkConstants.ImageSubPath + Path.GetFileName(theRes.mPath);
                }
...
#if ANDROID
                using (var st = SexyGlobal.gPvZActivity.Assets.Open(fontRes.mPath))
                {
        ...
        public bool TodLoadResources(string theGroup, bool doUnpackAtlasImages)
        {
            ...
            while (!SexyGlobal.gSexyAppBase.mShutdown && LoadNextResource())
            {
            }
            if (SexyGlobal.gSexyAppBase.mShutdown)
            {
                return false;
            }
            if (HadError())
            {
                SexyGlobal.gSexyAppBase.ShowResourceError(true);
                return false;
            }
        ...
        private static SpriteBatch imageLoadSpritebatch = new SpriteBatch(SexyGlobal.g.GraphicsDevice);
        ...
        internal class ImageRes : BaseRes
        {
            ...
            public override void DeleteResource()
            {
                ...
                {
                    ContentManager manager = SexyGlobal.gSexyAppBase.mContentManager;
                    if (mUnloadGroup > 0)
                    {
        ...
        internal class SoundRes : BaseRes
        {
            ...
            public override void DeleteResource()
            {
                if (mSoundId >= 0)
                {
                    SexyGlobal.gSexyAppBase.mSoundManager.ReleaseSound((uint)mSoundId);
                    mSoundId = -1;
                }
                base.DeleteResource();
            }
        ...
        internal class MusicRes : BaseRes
        {
            ...
            public override void DeleteResource()
            {
                if (mSongId >= 0)
                {
                    SexyGlobal.gSexyAppBase.mMusicInterface.UnloadMusic(mSongId);
                    mSongId = -1;
                }
                base.DeleteResource();
            }
