...
namespace Sexy
{
    ...
    public/*internal*/ class Dialog : Widget, ButtonListener
    {
        ...
        public Dialog(Image theComponentImage, Image theButtonComponentImage, int theId, bool isModal, string theDialogHeader, string theDialogLines, string theDialogFooter, int theButtonMode)
        {
            ...
            mIsModal = isModal;
            int num = (int)FrameworkConstants.InvertAndScale(24f);
            mContentInsets = new Insets(num, num, num, num);
            mTextAlign = 0;
            mLineSpacingOffset = 0;
            mSpaceAfterHeader = (int)FrameworkConstants.InvertAndScale(10f);
            mButtonSidePadding = 0;
            mButtonHorzSpacing = (int)FrameworkConstants.InvertAndScale(8f);
            mDialogListener = SexyGlobal.gSexyAppBase;
            ...
            mButtonHeight = ((theButtonComponentImage == null) ? ((int)FrameworkConstants.InvertAndScale(24f)) : theButtonComponentImage.mHeight);
        ...
        public virtual int GetPreferredHeight(int theWidth)
        {
            ...
            if (mDialogLines.Length > 0)
            {
                ...
                Graphics g = SexyGlobal.g;
                g.SetFont(mLinesFont);
                num += GetWordWrappedHeight(g, theWidth - mContentInsets.mLeft - mContentInsets.mRight - mBackgroundInsets.mLeft - mBackgroundInsets.mRight - (int)FrameworkConstants.InvertAndScale(4f), mDialogLines, mLinesFont.GetLineSpacing() + mLineSpacingOffset);
                flag = true;
                num += mSpaceAfterHeader;
            }
            if (mDialogFooter.Length != 0 && mButtonMode != 3)
            {
                if (flag)
                {
                    num += (int)FrameworkConstants.InvertAndScale(8f);
                }
        ...
        public override void MouseDrag(int x, int y)
        {
            if (mDragging)
            {
                var ss = SexyGlobal.gSexyAppBase.mScreenScales;
        ...
        {
            while (SexyGlobal.gSexyAppBase.UpdateApp() && mWidgetManager != null && mResult == 2147483647)
            {
            }
            if (autoKill)
            {
                SexyGlobal.gSexyAppBase.KillDialog(mId);
            }
            return mResult;
        }
