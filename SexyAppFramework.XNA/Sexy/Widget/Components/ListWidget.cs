...
namespace Sexy
{
    public/*internal*/ class ListWidget : Widget, ScrollListener
    {
        ...
        public override void OrderInManagerChanged()
        {
            ...
            if (mChild != null)
            {
                SexyGlobal.gSexyAppBase.mWidgetManager.PutInfront(mChild, this);
            }
            if (mScrollbar != null)
            {
                SexyGlobal.gSexyAppBase.mWidgetManager.PutInfront(mScrollbar, this);
            }
        }
        ...
        public override void Draw(Graphics g)
        {
            ...
            @new.ClipRect((int)FrameworkConstants.InvertAndScale(4f), (int)FrameworkConstants.InvertAndScale(4f), mWidth - (int)FrameworkConstants.InvertAndScale(8f), mHeight - (int)FrameworkConstants.InvertAndScale(8f));
            Graphics new2 = Graphics.GetNew(g);
            new2.ClipRect(0, (int)FrameworkConstants.InvertAndScale(4f), mWidth, mHeight - (int)FrameworkConstants.InvertAndScale(8f));
            @new.SetFont(mFont);
            ...
            {
                int num4 = (int)FrameworkConstants.InvertAndScale(4f) + (int)((i - mPosition) * height);
                ...
                {
                case 0:
                    theX = (int)FrameworkConstants.InvertAndScale(4f);
                    break;
                case 1:
                    theX = (mWidth - mFont.StringWidth(theString)) / 2;
                    break;
                default:
                    theX = mWidth - mFont.StringWidth(theString) - (int)FrameworkConstants.InvertAndScale(4f);
                    break;
                }
                @new.DrawString(theString, theX, num4 + num3 - (int)FrameworkConstants.InvertAndScale(5f));
            }
            if (mDrawOutline)
            {
        ...
        public override void MouseMove(int x, int y)
        {
            ...
            //x -= (int)absPos.x;
            //y -= (int)absPos.y;
            y -= (int)FrameworkConstants.InvertAndScale(4f);
            int num = (mItemHeight != -1) ? mItemHeight : mFont.GetHeight();
