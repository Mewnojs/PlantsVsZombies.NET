...
namespace Sexy
{
    public/*internal*/ class PrimitiveBatch : IDisposable
    {
        ...
        {
            ScreenScales s = SexyGlobal.gSexyAppBase.mScreenScales;
            float offsetX1 = (s.mTransX
                * (float)FrameworkConstants.BackBufferSize.X / device.PresentationParameters.BackBufferHeight);
            float offsetY1 = (s.mTransY
                * (float)FrameworkConstants.BackBufferSize.Y / device.PresentationParameters.BackBufferWidth);
            float offsetX2 = (device.PresentationParameters.BackBufferWidth / s.mScaleFactor) - FrameworkConstants.BackBufferSize.Y
                - offsetX1;
            float offsetY2 = (device.PresentationParameters.BackBufferHeight / s.mScaleFactor) - FrameworkConstants.BackBufferSize.X
                - offsetY1;
            basicEffect.View = Matrix.CreateOrthographicOffCenter(0f - offsetX1,
                FrameworkConstants.BackBufferSize.Y + offsetX2/*(float)this.device.PresentationParameters.BackBufferWidth*/,
                FrameworkConstants.BackBufferSize.X + offsetY2/*(float)this.device.PresentationParameters.BackBufferHeight*/,
                0f - offsetY1, 0f, 1f);
        ...
        {
            SexyGlobal.g.GraphicsDevice.RasterizerState = rasterizerState;
            if (hasBegun)
            {
