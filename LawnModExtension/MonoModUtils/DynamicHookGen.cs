            ...
            foreach (Tuple<ActionType, Delegate> action in Actions)
            {
                ...
                case ActionType.Add:
                    if (hookType == HookType.IL)
                        HookEndpointManagerNew.Modify(method, target);
                    else
                        HookEndpointManagerNew.Add(method, target);
                    break;

                case ActionType.Remove:
                    if (hookType == HookType.IL)
                        HookEndpointManagerNew.Unmodify(method, target);
                    else
                        HookEndpointManagerNew.Remove(method, target);
                    break;
                }
            }
